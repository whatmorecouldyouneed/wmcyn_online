...
3. For First Signal, use AFCore as the framework baseline, then map WMCYN-specific identity, roles, camera operator, Verbatim marker, and Crib world logic around it.
...
| BP_GameInstance_Main | `Content/AFCore/Blueprints/Game/BP_GameInstance_Main.uasset` | GameInstance | Config already uses this as `GameInstanceClass`; likely manages framework lifecycle/session-ish state. Binary contents not inspected. | Use or wrap; keep unless a WMCYN-specific GameInstance is proven necessary. |
| BP_PlayerController_Main | `Content/AFCore/Blueprints/Game/BP_PlayerController_Main.uasset` | Controller | Main AFCore player controller candidate for both PC and VR input routing. Binary contents not inspected. | Use as baseline; wrap WMCYN role/session behavior outside it if possible. |
| BP_PlayerState_Main | `Content/AFCore/Blueprints/Game/BP_PlayerState_Main.uasset` | Multiplayer | PlayerState is relevant for display names, roles, and replicated session identity. Binary contents not inspected. | Investigate manually; likely wrap/extend for WMCYN display name and role. |
...
| BP_Pawn_Base_Screen | `Content/AFCore/Blueprints/Pawn/BP_Pawn_Base_Screen.uasset` | Pawn | Screen-oriented pawn variant may be useful for desktop/operator views. Binary contents not inspected. | Investigate manually. |
| BP_Pawn_Desktop | `Content/AFCore/Blueprints/Pawn/Desktops/BP_Pawn_Desktop.uasset` | Pawn | Desktop pawn candidate for PC build and possible production operator mode. Binary contents not inspected. | Use/investigate for PC operator baseline. |
| BP_Pawn_Desktop_Camera | `Content/AFCore/Blueprints/Pawn/Desktops/BP_Pawn_Desktop_Camera.uasset` | Pawn | Explicit desktop camera pawn candidate for production camera and OBS capture workflow. Binary contents not inspected. | High priority: investigate manually for CameraOperator role. |
| BP_Pawn_Desktop_Char | `Content/AFCore/Blueprints/Pawn/Desktops/BP_Pawn_Desktop_Char.uasset` | Pawn | Desktop character pawn may support non-VR operator/guest testing. Binary contents not inspected. | Investigate manually. |
| BP_Pawn_VR | `Content/AFCore/Blueprints/Pawn/VR/BP_Pawn_VR.uasset` | Pawn | Main VR pawn candidate for Quest host/guest. Binary contents not inspected. | High priority: use/investigate for Quest roles. |
...
| BP_Pawn_Assymetric | `Content/AFCore/Blueprints/Pawn/Assymetric/BP_Pawn_Assymetric.uasset` | Pawn | Asymmetric pawn set may support PC+VR mixed roles. Binary contents not inspected. | Investigate manually for PC operator plus Quest participants. |
| BP_Pawn_Assymetric_Camera | `Content/AFCore/Blueprints/Pawn/Assymetric/BP_Pawn_Assymetric_Camera.uasset` | Pawn | Asymmetric camera pawn sounds directly aligned with production camera needs. Binary contents not inspected. | High priority: investigate manually for CameraOperator. |
| BP_Pawn_Assymetric_Char | `Content/AFCore/Blueprints/Pawn/Assymetric/BP_Pawn_Assymetric_Char.uasset` | Pawn | Asymmetric character pawn may support mixed desktop/VR sessions. Binary contents not inspected. | Investigate manually. |
...
| Comp_Replication | `Content/AFCore/Blueprints/Components/Multiplayer/Comp_Replication.uasset` | Multiplayer | Replication component candidate for shared session state. Binary contents not inspected. | High priority: investigate for private session and presence replication. |
...
| BP_LoginCredentials | `Content/AFCore/Blueprints/Multiplayer/BP_LoginCredentials.uasset` | Multiplayer | Login credential object may support user/session identity. Binary contents not inspected. | Investigate manually; do not assume backend auth. |
| BP_LoginCredentials_Simple | `Content/AFCore/Blueprints/Multiplayer/BP_LoginCredentials_Simple.uasset` | Multiplayer | Simpler identity/login option may fit First Signal better than full auth. Binary contents not inspected. | High priority: investigate for display name only. |
| BP_SessionInfo | `Content/AFCore/Blueprints/Multiplayer/BP_SessionInfo.uasset` | Multiplayer | Session state object candidate. Binary contents not inspected. | Investigate for private session metadata. |
| BP_SessionInfo_Simple | `Content/AFCore/Blueprints/Multiplayer/BP_SessionInfo_Simple.uasset` | Multiplayer | Simple session info likely better for First Signal than complex backend integration. Binary contents not inspected. | High priority: investigate for private PC+Quest session. |
...
| Comp_PawnUI | `Content/AFCore/Blueprints/Components/UI/Comp_PawnUI.uasset` | Widget | Pawn-attached UI component candidate for names/roles. Binary contents not inspected. | Wrap for display name/role labels if suitable. |
...
| BP_PawnUI_Element | `Content/AFCore/Blueprints/UI/PawnUI/BP_PawnUI_Element.uasset` | Widget | Likely a pawn label/UI element. Binary contents not inspected. | High priority for display name and role tags. |
| Widget_SelectionWindow | `Content/AFCore/Blueprints/UI/SelectionWindow/Widget_SelectionWindow.uasset` | Widget | Selection/menu UI may support role/session/debug selection. Binary contents not inspected. | Investigate, but keep First Signal UI minimal. |
| Widget_Keyboard | `Content/AFCore/Blueprints/UI/Keyboard/Widgets/Widget_Keyboard.uasset` | Widget | In-world keyboard may support entering display names/session names in VR. Binary contents not inspected. | Investigate if private session naming is needed. |
...
| BP_Helper_Spectator | `Content/AFCore/Blueprints/Helpers/BP_Helper_Spectator.uasset` | Other | Spectator helper may support production camera/operator monitoring. Binary contents not inspected. | High priority: investigate for OBS/camera workflow. |
| Comp_Spectator | `Content/AFCore/Blueprints/Components/Misc/Comp_Spectator.uasset` | Other | Spectator component may support non-participant camera/operator mode. Binary contents not inspected. | High priority: investigate. |
...
| DA_ControllerVR_OculusPreset | `Content/AFCore/DataAsset/Presets/VRControls/General/DA_ControllerVR_OculusPreset.uasset` | Controller | Quest/Oculus control preset candidate. Binary contents not inspected. | High priority for Quest 3 controls. |
...
| SK_Hand_Oculus_L / SK_Hand_Oculus_R | `Content/AFCore/Meshes/Pawn/Hand/Oculus/` | Pawn | Oculus hand meshes likely aligned with Quest controllers/hands. Binary contents not inspected. | High priority for Quest placeholder hands. |
...
| Map_Example_Quest | `Content/AFCore/Examples/Maps/Map_Example_Quest.umap` | Demo Map | Explicit Quest example map, strongly relevant to Quest 3 build. Binary contents not inspected. | High priority for Quest validation. |
...
| Map_Example_Menu | `Content/AFCore/Examples/Maps/Map_Example_Menu.umap` | Demo Map | Menu example may contain session/role/menu patterns. Binary contents not inspected. | Investigate, but keep First Signal menu minimal. |
| Map_Example_UserInterfaces | `Content/AFCore/Examples/Maps/Map_Example_UserInterfaces.umap` | Demo Map | UI demo likely shows widgets/overlays/in-world panels. Binary contents not inspected. | Investigate for display name, role UI, marker feedback. |
...
- Project settings target Mobile/Scalable and Android/Windows, with `bStartInVR=True` and Quest2 packaging enabled. That is favorable for First Signal's PC + Quest target, though Quest 3 should still be manually validated.
...
2. Validate `BP_Pawn_VR`, `BP_Pawn_VR_Char`, Oculus hand meshes, and hand AnimBPs for Quest host/guest.
3. Validate `BP_Pawn_Desktop_Camera`, `BP_Pawn_Assymetric_Camera`, `BP_Helper_Spectator`, `Comp_Spectator`, and `BP_Manager_Spectator` for PC production operator / OBS capture.
4. Validate `BP_SessionInfo_Simple`, `BP_LoginCredentials_Simple`, `Comp_Replication`, and `Comp_RPC` for private session, display names, roles, and the Verbatim marker event.
...
- Use: AFCore GameMode, GameInstance, PlayerController, VR/Desktop/Asymmetric pawn families, controller presets, hand assets, and example maps as the baseline.
- Wrap: PlayerState/session/login/replication/UI pieces for WMCYN display name, roles, camera operator, Verbatim marker, and world events.
...
- Online subsystem is currently `Null`, so PC + Quest private session may work only as local/LAN-style testing until a real online subsystem path is chosen.
