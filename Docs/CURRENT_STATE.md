...
- After promotion to the native production pawn, its inherited rays were found to trace `Pawn` instead of the UI `Visibility` channel. The WMCYN runtime adapter corrects that mismatch. The local right-hand pointer is attached to Manny's `index_03_r` fingertip, uses a thin cyan debug line, and displays only while its trace is over a widget component.
