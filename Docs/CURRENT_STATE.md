...
- After promotion to the native production pawn, its inherited rays were found to trace `Pawn` instead of the UI `Visibility` channel. The WMCYN runtime adapter corrects that mismatch. The local right-hand pointer is attached to Manny's `index_03_r` fingertip with a `180` degree local yaw so component `+X` traces outward rather than back toward the hand. It uses a thin cyan debug line, remains visible for the entire login gate, and hides immediately after successful login.
