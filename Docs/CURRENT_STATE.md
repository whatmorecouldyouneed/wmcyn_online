...
- After promotion to the native production pawn, its inherited rays were found to trace `Pawn` instead of the UI `Visibility` channel. The WMCYN runtime adapter corrects that mismatch and now shows the local right-hand pointer as a cyan debug ray.
- The native pawn can open the AFCore keyboard, but it does not participate in AFCore's controller registration path. That left `Comp_Widget.lastWidgetInteractionComponent` empty, so keyboard keys could highlight while `SendKeyChar`, Enter, and Close had no source pointer. `WBP_WMCYN_LoginJoin` now registers the WMCYN adapter's preferred right-hand interaction with the AFCore widget host before spawning or refocusing the keyboard. The production native-pawn headset click/type/Enter/Close regression remains the next hardware confirmation.
