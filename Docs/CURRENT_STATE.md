...
- After promotion to the native production pawn, its inherited rays were found to trace `Pawn` instead of the UI `Visibility` channel. The WMCYN runtime adapter corrects that mismatch. The local right-hand pointer is attached to Manny's `index_03_r` fingertip and uses a thin cyan debug line. It remains visible for the entire login gate so the user can aim before acquiring a widget, then hides immediately after successful login.
