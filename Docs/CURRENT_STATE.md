...
- Quest test accounts exist in Firebase Auth for AA (`vantageforever8@gmail.com`) and ghost (`mbatagio123@gmail.com`); their `users/{uid}` and `loginHandles` Firestore docs are pending manual console creation, so bootstrap currently returns null usernames for them.
