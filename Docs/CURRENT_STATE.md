...
- The native pawn uses `WMCYN_Nameplate_Runtime`, a lightweight WMCYN-owned text component attached above the head. Successful login refreshes and enables it immediately. `Owner No See` keeps it out of the user's direct HMD view while allowing the local mirror and remote users to render it. It reads replicated `DisplayName` with standard PlayerName as fallback.
