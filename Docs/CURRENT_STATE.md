...
- After promotion to the native production pawn, its inherited rays were found to trace `Pawn` instead of the UI `Visibility` channel. The WMCYN runtime adapter corrects that mismatch. The local right-hand pointer is first aligned from Manny's `index_03_r` fingertip with a `180` degree local yaw, then re-parented with that pose preserved to the stable `hand_r` bone. The cyan ray therefore appears at the index finger and traces outward without following trigger-finger curl during a click. It remains visible for the entire login gate and hides immediately after successful login.
