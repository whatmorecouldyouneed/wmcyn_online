...
- AFCore's keyboard sends characters through the overlay's source `WidgetInteractionComponent`. Because the native Mimic pawn does not populate AFCore `Comp_Widget.lastWidgetInteractionComponent`, register the WMCYN adapter's preferred right-hand pointer with the login widget host before every keyboard spawn or field switch. Keep this compatibility bridge WMCYN-owned.
- Keep the local right-hand widget pointer visible as a cyan debug ray through the First Signal interaction-validation gate. Remove or replace the debug visualization only after headset login interaction is repeatably proven.
