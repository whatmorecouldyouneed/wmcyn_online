...
- Align the local right-hand pointer from Manny's `index_03_r` fingertip with a `180` degree local yaw, then preserve that pose while re-parenting it to `hand_r`. Do not leave the interaction component attached to the animated fingertip bone because trigger-finger curl moves the ray during `PressPointerKey`. Keep its cyan debug line thin and scoped to the login gate, then hide it immediately after successful login.
