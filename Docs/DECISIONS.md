...
- Position every runtime nameplate from the replicated HMD camera plus a fixed world-up offset. Do not inherit the imported `Top of Head` helper's rotation because it can move the label sideways; keep the text viewer-facing and readable for Quest and PCVR clients.
