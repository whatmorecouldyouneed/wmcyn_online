...
- Attach the local right-hand pointer to Manny's `index_03_r` fingertip. Keep its cyan debug line thin and contextual: show it only while the trace is over a world widget, then remove or replace the debug visualization after headset login interaction is repeatably proven.
