...
- Attach the local right-hand pointer to Manny's `index_03_r` fingertip and apply a `180` degree local yaw so the widget interaction component's `+X` trace points away from the hand. Keep its cyan debug line thin and scoped to the login gate: show it from gate open through identity submission, then hide it immediately after successful login. Remove or replace the debug visualization after headset login interaction is repeatably proven.
