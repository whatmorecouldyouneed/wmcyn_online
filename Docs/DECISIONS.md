...
- Attach the local right-hand pointer to Manny's `index_03_r` fingertip. Keep its cyan debug line thin and scoped to the login gate: show it from gate open through identity submission so the user can aim before hover is acquired, then hide it immediately after successful login. Remove or replace the debug visualization after headset login interaction is repeatably proven.
